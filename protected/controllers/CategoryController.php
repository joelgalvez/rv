...
class CategoryController extends CController
{
	...
	{
		$this->render('show',array('model'=>$this->loadcategory()));
	}
	...
	public function actionCreate()
	{
		$model=new category;
                ...
                $criteria->select='id,name';  // only select the 'title' column
                $criteria->condition='parentId=0';
                //$criteria->params=array(':postID'=>10);
                $parentlist= Category::model()->findAll($criteria);
                $aParentlist = null;
                //THey dont want it
                //$aParentlist['0'] ='ROOT';
	...
	public function actionAdmin()
	{
		...
		$pages=new CPagination(category_v::model()->count($criteria));
		$pages->pageSize=self::PAGE_SIZE;
		$pages->applyLimit($criteria);
		...
		$models=category_v::model()->findAll($criteria);

		$this->render('admin',array(
			'models'=>$models,
			'pages'=>$pages,
			'sort'=>$sort,
                        'tree'=>category_v::model()->getTree(),
		));
	}
